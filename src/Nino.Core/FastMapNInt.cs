...
{
    /// <summary>
    /// Fast hash map optimized for nint keys using cuckoo hashing.
    ...
    public sealed class FastMapNInt<TValue> : IDisposable, IEnumerable<KeyValuePair<nint, TValue>>
    {
        ...
        public ref TValue GetValueRefOrAddDefault(nint key, out bool exists)
        {
            ...
            else
            {
                ...
                // Key was just added, look it up directly from the tables
                var addedIndex1 = (int)(key & _capacityMask);
                ref Entry addedEntry1 = ref _table1[addedIndex1];
                if (addedEntry1.IsOccupied && addedEntry1.Key == key)
                    return ref addedEntry1.Value;

                var addedIndex2 = (int)((key >> 8) & _capacityMask);
                ref Entry addedEntry2 = ref _table2[addedIndex2];
                return ref addedEntry2.Value;
            }
        }
